...
const uint32_t ENCCONF_ADR = writeAccess + TMCRhino_ENCMODE;	//ADDED ENCODER STUFF
const uint32_t ENCCONST_ADR = writeAccess + TMCRhino_ENC_CONST; //ADDED ENCODER STUFF
const uint32_t XENC_ADR = writeAccess + TMCRhino_XENC;			//ADDED ENCODER STUFF
const uint32_t SWMODE_ADR = writeAccess + TMCRhino_SWMODE;		//ADDED ENCODER STUFF  TMCRhino_RAMPSTAT 
const uint32_t RAMPSTAT_ADR = writeAccess + TMCRhino_RAMPSTAT; //Added endstop lock clearing 
...
public:
	...
	int32_t ENCACTUAL, VACTUAL, XACTUAL, XACTUAL_OLD, XTARGET;
	float xrad = 2 * 3.141592653589793 / 4.; // position goal in units of the international system, here, 6.28 rad (one round)
	struct SPI_READOUT
	{
		bool status_stop_r,
			status_stop_l,
	...
	void configENCCONF(uint32_t datagram); //encoder stuff
	void configSWMODE(uint32_t datagram);  //endstop settings
	void resetENCXPOS();

	int32_t getVACTUAL();
	int32_t getXACTUAL();
	int32_t getENCMODE();
	int32_t getRAMP_STAT();
	int32_t getXENC();
	void configRAMPMODE(uint32_t datagram);
	void readallSPI(uint32_t address, uint32_t datagram);
	void readStatByte(uint8_t tempByte);
	void printStatByte();
	void homeAxis();
	void motionTracking(int32_t xtargetMoTrack);
	void EncReset();
	void Readout(int ReadoutSamples);
	void zeroAxis();
	void MaxAxis();
	void RAMPSTAT_RESET();
	void updateReadings();
	...
	void setENCCONST(uint32_t user_Enc_CONST);
...
private:
	...
	uint32_t _currentENCCONFDatagram;  //ADDED ENC STUFF
	uint32_t _currentENCCONSTDatagram; //ADDED ENC STUFF
	uint32_t _currentSWMODEDatagram;
